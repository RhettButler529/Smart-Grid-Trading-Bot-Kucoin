...
(function() {
    ...
    mraidview.render = function () {
        ...
        if (!adFrame || !adWindow || !adWindow.document || !adFrame.contentWindow) {
            ...
            adWindow = window.open((offscreen) ? 'safari/device-pages.html': 'https://www.kucoin.com/trade/ETH-USDT?spm=kcWeb.B1ContractPro.Header4.1', 'adWindow', 'left=1000,width='+screenSize.width+',height='+screenSize.height+',menubar=no,location=no,toolbar=no,status=no,personalbar=no,resizable=no,scrollbars=no,chrome=no,all=no');
