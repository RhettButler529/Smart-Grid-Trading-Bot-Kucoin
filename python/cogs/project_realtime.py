...
class project_realtime(cogcore):
  @cog_ext.cog_slash(name="punk_realtime",
  description="return some useful realtime information from the project from OpenSea API",  
  )

  async def project_realtime(self,ctx):
    url0='https://api.opensea.io/api/v1/collection/'+config('project_name')+'?format=json' #api url
    ...
    url1='https://api.opensea.io/api/v1/collection/'+config('project_name')+'/stats?format=json' #api url
    ...
    if(total_volume != 0):
      embed=discord.Embed(title="["+config('project_name')+" realtime information]", color=0xe8006f)
